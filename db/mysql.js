    ...
    async execute(sql, params = []){
      ...
      return new Promise((resolve, reject)=>{
        ...
        if (first === 'SELECT'){
          ...
          db.run(sql, params, function(err){ if (err) return reject(err); resolve([{ affectedRows: this.changes, lastID: this.lastID }]); });
        }
      });
    },
    close(){ db.close(); }
...
module.exports = {
  ...
  get isSqlite(){ return dbInstance && dbInstance.isSqlite; },
  close(){ if (dbInstance && dbInstance.close) return dbInstance.close(); return Promise.resolve(); }
};
